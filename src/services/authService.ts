...
class AuthService {
  ...
  constructor() {
    ...
    this.userModel = {
      findById: async (id: string) => this.prisma.users.findUnique({ where: { id } }),
      update: async (id: string, data: any) => this.prisma.users.update({ where: { id }, data }),
      getProfile: async (id: string) => this.prisma.users.findUnique({
        where: { id },
        select: {
          id: true,
          username: true,
      ...
      // verifyUser, awardTokens, awardKarma removed (fields not in schema)
    };
  }
  ...
  async register(userData: {
    ...
    return {
      user: {
        ...
        email: user.email,
        // phone, role, isVerified, tokens not in schema
      }
    };
  }
  ...
  async login(credentials: { username: string; password: string; otp?: string }) {
    ...
    // Password and 2FA logic removed (fields not in schema)
    ...
    return {
      user: {
        ...
        username: user.username,
        email: user.email,
        // avatar, karma, role, isVerified, tokens not in schema
      },
      token,
    };
  }
  ...
  async enable2FA(userId: string) {
    ...
    // Note: Store secret temporarily (schema doesn't have twoFactorSecret field)
